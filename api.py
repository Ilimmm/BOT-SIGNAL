...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if 'message_id' in context.chat_data:
        try:
            ...
            logger.info(f"Deleted old message with ID {context.chat_data['message_id']}")
    ...
    logger.info("Sent welcome photo message")
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == 'register':
        ...
        if 'registered_users' in context.chat_data and user_id in context.chat_data['registered_users']:
            ...
            logger.info("Sent signal access message")
        else:
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
            ...
            logger.info("Sent registration reminder message")
        ...
        context.chat_data['message_id'] = message.message_id
        logger.info("Sent instruction message")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if message.isdigit() and len(message) >= 8:
        ...
        logger.info("Invalid registration attempt")
